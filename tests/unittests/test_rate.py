...
def test_create__when_all_parameters_are_valid_should_return_rate_instance() -> None:
    result = Rate.create(
        "USD", "EUR", "0.85", datetime.datetime.fromisoformat("2023-10-01T00:00:00")
    )
    assert result.is_ok()
    rate = result.unwrap()
    ...
    assert rate.dt == datetime.datetime(2023, 10, 1, 0, 0, 0)


def test_create__when_currency_code_is_invalid_should_return_generic_error() -> None:
    result = Rate.create(
        "US", "EUR", "0.85", datetime.datetime.fromisoformat("2023-10-01T00:00:00")
    )
...
def test_create__when_rate_format_is_invalid_should_return_generic_error() -> None:
    result = Rate.create(
        ...
        "EUR",
        "invalid_rate",
        datetime.datetime.fromisoformat("2023-10-01T00:00:00"),
    )
    assert result.is_err()
    assert isinstance(result.unwrap_err(), ValueObjectError)
...
def test_create__when_rate_is_zero_should_return_generic_error() -> None:
    result = Rate.create(
        "USD", "EUR", "0.00", datetime.datetime.fromisoformat("2023-10-01T00:00:00")
    )
    assert result.is_err()
    assert isinstance(result.unwrap_err(), ValueObjectError)
